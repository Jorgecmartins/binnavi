...
public class ARMUBFXTranslator extends ARMBaseTranslator {
  @Override
  protected void translateCore(final ITranslationEnvironment environment,
      final IInstruction instruction, final List<ReilInstruction> instructions) {
    final String operand =
        instruction.getOperands().get(1).getRootNode().getChildren().get(0).getValue();
    final Integer lsb = Integer.parseInt(
        instruction.getOperands().get(2).getRootNode().getChildren().get(0).getValue());
    final Integer width = Integer.parseInt(
        instruction.getOperands().get(3).getRootNode().getChildren().get(0).getValue());
    ...
    if (msb <= 31) {
      ...
      instructions.add(
          ReilHelpers.createAnd(baseOffset++, dw, operand, dw, String.valueOf(mask), dw, tempVar1));
    } else {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    translateAll(environment, instruction, "UBFX", instructions);
  }
}
