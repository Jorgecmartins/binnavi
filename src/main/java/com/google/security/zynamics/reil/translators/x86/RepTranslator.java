...
public class RepTranslator implements IInstructionTranslator {
  private final IStringInstructionGenerator translator;
  private final OperandSize operandSize;

  public RepTranslator(final IStringInstructionGenerator translator, final OperandSize operandSize) {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    final String jmpGoal =
        String.format("%d.%d", instruction.getAddress().toLong(), innerInstructions.size() + 5);
    instructions
    ...
    instructions.add(ReilHelpers.createJcc(baseOffset + instructions.size(), OperandSize.DWORD,
        "1", OperandSize.ADDRESS, firstInstruction));
