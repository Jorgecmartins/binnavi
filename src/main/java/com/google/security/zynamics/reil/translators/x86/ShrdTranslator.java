...
/**
 * Translates SHRD instructions to REIL code.
 */
public class ShrdTranslator implements IInstructionTranslator {
  ...
  private String getAllButMask(final OperandSize extendedSize, final OperandSize combinedSize) {
    if (extendedSize == OperandSize.QWORD) {
      return String.valueOf(0xFFFFFFFF00000000L);
    } else {
  ...
  public void translate(final ITranslationEnvironment environment, final IInstruction instruction,
    ...
    if (instruction.getOperands().size() != 3) {
      throw new InternalTranslationException(
          "Error: Argument instruction is not a shrd instruction (invalid number of operands)");
    }
    ...
    instructions.add(ReilHelpers.createBsh(offset++, secondResult.getSize(),
        secondResult.getRegister(), secondResult.getSize(), shiftMask, combinedSize,
        shiftedSecondInput));
    instructions.add(ReilHelpers.createOr(offset++, combinedSize, shiftedSecondInput,
        firstResult.getSize(), firstResult.getRegister(), combinedSize, combinedSource));
    ...
    instructions.add(ReilHelpers.createSub(offset++, OperandSize.BYTE, "0", OperandSize.BYTE,
        truncatedShiftValue, OperandSize.WORD, negatedShiftMask));
    instructions.add(ReilHelpers.createAnd(offset++, OperandSize.WORD, negatedShiftMask,
        OperandSize.BYTE, "255", OperandSize.BYTE, truncatedNegatedShiftMask));
    ...
    instructions.add(ReilHelpers.createAdd(offset++, OperandSize.BYTE, truncatedNegatedShiftMask,
        OperandSize.BYTE, "1", OperandSize.WORD, incShiftMaskNeg));
    instructions.add(ReilHelpers.createBsh(offset++, combinedSize, combinedSource,
        OperandSize.WORD, incShiftMaskNeg, combinedSize, decResult));
    instructions.add(ReilHelpers.createAnd(offset++, combinedSize, decResult, OperandSize.BYTE,
        "1", OperandSize.BYTE, Helpers.CARRY_FLAG));

    // Do the shift

    instructions.add(ReilHelpers.createBsh(offset++, combinedSize, combinedSource,
        OperandSize.BYTE, truncatedNegatedShiftMask, extendedSize, shiftedResult));
